...
$IMG_URL = "/Neo/theme/basic/img/mobile/data-img/sightseeing/seogwipo-west/";
...
<div id="td" style="width:100%;">
  <div id="title">
    <div class="sub_title" style="font-size:11px;">
      <b>서귀포시 서쪽</b>
    </div>
    <div class="sub_title">
      <b>관광</b>
...
if($conn->connect_errno){
  ...
  $response = $conn->query("select * from ` Seogwipo west`");
  if($response){
    while($row = $response->fetch_assoc()){
      $imgData = explode(",", $row["image-folder"]);
      ?>
      <ul id="ul">
        <a href="/Neo/theme/basic/mobile/shop/sightseeing_data/seogwipo-west_detail.php?name=<?php echo $row["name"]?>&address=<?php echo $row["address"]?>number=<?php echo $row["number"]?>&working time=<?php echo $row["working time"]?>&explanation=<?php echo $row["explanation"]?>&price=<?php echo $row["price"]?>&Lat=<?php echo $row["Lat"]?>&Lon=<?php echo $row["Lon"]?>">
          <li id="datalist2">
