<?php
$IMG_URL = "/Neo/theme/basic/img/mobile/data-img/sightseeing/seogwipo-east/";
...
if($conn->connect_errno){
  ...
  $response = $conn->query("select * from ` Seogwipo east`");
  if($response){
    while($row = $response->fetch_assoc()){
      $imgData = explode(",", $row["image-folder"]);
      ?>
      <ul id="ul">
        <a href="/Neo/theme/basic/mobile/shop/sightseeing_data/seogwipo-east_detail.php?name=<?php echo $row["name"]?>&address=<?php echo $row["address"]?>number=<?php echo $row["number"]?>&working time=<?php echo $row["working time"]?>&explanation=<?php echo $row["explanation"]?>&price=<?php echo $row["price"]?>&Lat=<?php echo $row["Lat"]?>&Lon=<?php echo $row["Lon"]?>">
          <li id="datalist2">
